...
<img src="./images/terraform-applying-kubernetes-over-aws-succ.png" alt="Terraform applying Kubernetes infrastructure on AWS" width="800">
...
<img src="./images/Dockerhub-app.png" alt="DockerHub image repository" width="800">

**2 — Kubernetes manifests applied to the live cluster:**

<img src="./images/project-kubernetes-from-dockerhub.png" alt="Kubernetes deployment from DockerHub" width="800">

**3 — Application live on public AWS ELB URL, connected to PostgreSQL ✅**

<img src="./images/app-working-on-AWS-talking-to-DB.png" alt="Application running on AWS and connected to PostgreSQL" width="800">
...
- A declarative `Jenkinsfile` that defines every stage of the delivery lifecycle — the pipeline lives in the repo alongside the code
...
<img src="./images/error.png" alt="Jenkins Docker socket permission error" width="800">
...
The `kube-prometheus-stack` requires significant RAM. On `t3.micro` free-tier nodes, monitoring pods stayed in `Pending` state — the nodes did not have enough memory or available ENI capacity to schedule them.
...
**Fix:** Upgraded the node group to `t3.medium` instances. I also switched the application Deployment strategy from `RollingUpdate` to `Recreate` during resource-constrained testing to avoid running old and new pods simultaneously.

**Why this matters:** This is exactly the kind of capacity planning decision that separates engineers who have run workloads in real cloud environments from those who have only worked locally.
...
|-----------|------------|----------|
| App could not connect to DB locally | Containers on different default networks | Defined a shared named network in `docker-compose.yml` |
| Docker build failed inside Jenkins | Docker socket permission denied | Mounted `/var/run/docker.sock` and added Jenkins user to the Docker group |
| Monitoring pods stuck in `Pending` | `t3.micro` had insufficient RAM and ENI capacity | Upgraded node group to `t3.medium` |
| Both old and new pods OOM during rollout | `RollingUpdate` temporarily doubled pod count | Switched to `Recreate` strategy during constrained deployments |
| EKS `kubectl` access from Jenkins | Jenkins had no cluster permissions | Used IAM role access and RBAC binding through `aws-auth` |
...
- **Real troubleshooting** — diagnosed and fixed Docker socket permissions, resource constraints, and network issues in a live cloud environment
